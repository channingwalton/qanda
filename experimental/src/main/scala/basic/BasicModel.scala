...
object BasicModel {
  ...
  val questionnaire: List[Question[_]] = StringQuestion("a", "Who are you?") :: BooleanQuestion("b", "Happy?") :: AddressQuestion(
    "c",
    "Where do you live?"
  ) :: Nil
  ...
  def html[T](question: Question[T]): NodeSeq =
    question match {
      ...
      case AddressQuestion(k, q) =>
        <span>{q}:</span> ++ <input id={s"$k-line1"} /> ++ <input id={s"$k-line2"} /> ++ <input id={s"$k-etc"} />
    }
